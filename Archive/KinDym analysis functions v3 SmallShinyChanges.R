###################################
# Changes compared to orginal v3
#
# - Added With Prgress bars to plotting funcitons
########################################################################
...
relatedness_GAM.plot = function(all.living.df, sex.to.plot, return.graph = TRUE, return.data = FALSE){
  if(sex.to.plot == "both"){ # "both" plots the comapritive kinship dynamics of males and females to an average memeber of the group 
    ...
    withProgress(message = "Fitting GAM",min = 0, max =3 ,value = 0, { 
      incProgress(amount = 1 ,detail = "1/2")
      print("Calculating GAM 1/2")
      fgam = gam(local.relatedness ~ s(age, k = 4) + s(id, bs = "re"), #s(age, k = 4)
                 data = fs, 
                 family=betar(link="logit"),
                 method = "REML")
      
      print("Calculating GAM 2/2")
      incProgress(amount = 1 ,detail = "2/2")
      mgam = gam(local.relatedness ~ s(age, k = 4) + s(id, bs = "re"), # s(age, k = 4)
                 data = ms, 
                 family=betar(link="logit"),
                 method = "REML")
    ...
    withProgress(message = "Fitting GAM",min = 0, max =3 ,value = 0, { 
      ...
      gam.toAll = gam(local.relatedness ~ s(age, k = 4) + s(id, bs = "re"), #, k = 4
                      data = onesexdf, 
                      family=betar(link="logit"),
                      method = "REML")
      ...
      gam.toFs = gam(relatedness.to.Fs ~ s(age, k = 4) + s(id, bs = "re"), #, k = 4
                     data = onesexdf, 
                     family=betar(link="logit"),
                     method = "REML")
      ...
      gam.toMs = gam(relatedness.to.Ms ~ s(age, k = 4) + s(id, bs = "re"), #, k = 4
                     data = onesexdf, 
                     family=betar(link="logit"),
                     method = "REML")
...
# calc_ymax = function(plotdata.df){
#   max.relatedness = max(plotdata.df$relatedness)+plotdata.df$r.se[plotdata.df$relatedness == max(plotdata.df$relatedness)]
#   # if(max.relatedness > 0.5){
#   #   ymax = 1
#   # } else {
#   #   if(max.relatedness >0.25){
#   #     ymax = 0.5
#   #   } else {
#   #     if(max.relatedness >0.1){
#   #       ymax = 0.25
#   #     } else {
#   #       ymax = 0.1
#   #     }
